...
const CRAWL_LIST_URL =
  "http://www.slrclub.com/bbs/zboard.php?id=best_article&category=1&setsearch=category";

const getListItem = async (crawl_list_length, crawl_item_count) => {
  let best_dom_list = document.querySelectorAll("#bbs_list tbody > tr");
  const slice_num =
    best_dom_list.length + crawl_list_length <= crawl_item_count
      ? best_dom_list.length
      : crawl_item_count - crawl_list_length;

  best_dom_list = [...best_dom_list].slice(0, slice_num);

  const result_list = best_dom_list
    .map(dom_item => {
      try {
...
const crawlItems = lib.makeCrawlItems(
  crawlItem,
  "#userct",
  "#slrct #userct img"
);
