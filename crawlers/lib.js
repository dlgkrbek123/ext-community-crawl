  ...
  return async (page, crawl_item_count) => {
    ...
    try {
      ...
      while (crawl_list.length !== crawl_item_count) {
        const cur_crawl_list = await page.evaluate(getListItem, crawl_list.length, crawl_item_count);
...
const testCrawlItem = (items) => {
  items.map((item) => {
    ...
    const valid_all = valid_src && valid_title && valid_view_num && valid_comment_num && valid_post_time

    if (!valid_all) {
      throw new Error(`find error validating post page (${src})\n ${JSON.stringify(item)}`)
    }
  })
}

exports.makeCrawler = (name, getList, crawlItems, refineItems) => async (crawl_item_count) => {
